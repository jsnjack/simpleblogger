...
def send_post(post_title, post_text, labels, blogid, username, password):
    result, blogger_service = login(username, password)
...
def login_image(username, passw):
    picasa_client = gdata.photos.service.PhotosService()
    picasa_client.email = username+'@gmail.com'
    picasa_client.password = passw
    picasa_client.source = 'jsn-simpleblogger'
    picasa_client.ProgrammaticLogin()
    albums = picasa_client.GetUserFeed(user=username)
    return albums, picasa_client


def upload_photo(picasa_client, album_url, image_name, path):
